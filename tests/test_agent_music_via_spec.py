...
@pytest.mark.asyncio
async def test_agent_runs_heartmula_tool_via_spec(monkeypatch, tmp_path):
    ...
    # Use example agent spec that includes 'music' agent with tools_allowlist
    import app.agent_runtime_v1 as agent_rt
    example = Path(__file__).resolve().parents[1] / "env" / "agent_specs.json.example"
    monkeypatch.setattr(config.S, "AGENT_SPECS_PATH", str(example))
